...
der)
  for memory in 200 500 1000 2000
    ...
    do
      ...
      do
        python main.py +scenario=cil_cifar10_5 model="$MODEL" +training=cifar10_5 +method=der_default device=$DEVICE head=linear +head.initial_out_features=100 method.alpha=$alpha method.beta=$beta experiment=dev method.mem_size=$memory hydra=search +wadnb_tags=[grid_search]
      done
    done
  done
;;
