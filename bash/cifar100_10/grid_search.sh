...
der)
  for memory in 200 500 1000 2000
    ...
    do
      ...
      do
        python main.py +scenario=cil_cifar100_10 +model="$MODEL" +training=cifar10_5 +method=der_default optimizer=adam device=$DEVICE +model.head_classes=100 method.alpha=$alpha method.beta=$beta experiment=dev method.mem_size=$memory hydra=search
      done
    done
  done
;;
