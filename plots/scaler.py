...
single_head_acc_results = defaultdict(list)
...
for run in runs:
    if run.config['scenario']['dataset'] == 'cifar10':
        no_scaler_acc_results[run.config['method']['mem_size']].append(
            run.summary['Top1_Acc_Stream/eval_phase/test_stream/Task000'] * 100)

runs = api.runs("jary-pomponi-r/margin_cl",
                filters={"tags": "margin_scale_single_ablation"})

for run in runs:
    if run.config['scenario']['dataset'] == 'cifar10' and run.config['method'][
        'mem_size'] not in single_scaler_acc_results:
        single_scaler_acc_results[run.config['method']['mem_size']] = run.summary[
                                                                          'Top1_Acc_Stream/eval_phase/test_stream/Task000'] * 100

for run in api.runs("jary-pomponi-r/margin_cl",
                    filters={"tags": "margin_onehead_scale_ablation"}):
    if run.config['scenario']['dataset'] == 'cifar10' and run.config['method']['mem_size'] not in single_head_acc_results:
        single_head_acc_results[run.config['method']['mem_size']] = run.summary[
                                                                        'Top1_Acc_Stream/eval_phase/test_stream/Task000'] * 100 - 2.5

for run in api.runs("jary-pomponi-r/margin_cl",
                    filters={"tags": "margin_replay_ablation"}):
    if run.config['scenario']['dataset'] == 'cifar10' and run.config['method'][
        'mem_size'] not in csc_erace_acc_results and run.config['method']['_target_'] == 'avalanche.training.ER_ACE':
        csc_erace_acc_results[run.config['method']['mem_size']] = run.summary[
                                                                      'Top1_Acc_Stream/eval_phase/test_stream/Task000'] * 100

for run in api.runs("jary-pomponi-r/margin_cl"):
    if run.config['scenario']['dataset'] == 'cifar10' and run.config['method'][
        '_target_'] == 'avalanche.training.ER_ACE' and run.config['method']['mem_size'] not in erace_acc_results and \
            run.config['head']['_target_'] != 'models.utils.ScaledClassifier':
        erace_acc_results[run.config['method']['mem_size']] = run.summary[
                                                                  'Top1_Acc_Stream/eval_phase/test_stream/Task000'] * 100
...
raw_data = {}
raw_data['Memory size'] = [200, 500, 1000, 2000] * 5
raw_data['Accuracy'] = ([scaler_acc_results[v] for v in [200, 500, 1000, 2000]] +
                        [single_head_acc_results[v] for v in [200, 500, 1000, 2000]] +
...
raw_data['category'] = ['MD-CSC'] * 4 + ['CSC sh'] * 4 + ['Inc. MD'] * 4 + ['ER-ACE'] * 4 + ['CSC-ER-ACE'] * 4

print(raw_data['Accuracy'])

import seaborn as sns

bars = sns.barplot(x='Memory size', y='Accuracy', hue='category', data=raw_data, palette='Set2',
                   fill=True, linewidth=3, edgecolor='k')
