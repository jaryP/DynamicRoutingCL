...
lode)
  for memory in 200 500 1000 2000
    ...
    do
      python main.py +scenario=cil_cifar100_10 model="$MODEL" +training=cifar10_5 +method=lode device=$DEVICE head=incremental method.rho=$rho experiment=dev method.mem_size=$memory hydra=search +wadnb_tags=[grid_search]
    done
  done
;;
margin)
...
do
  ...
  do
    ...
    python main.py +scenario=cil_cifar100_10 model="$MODEL" +training=cifar100 +method=margin head=margin_head device=$DEVICE method.mem_size=$memory method.past_task_reg=$past_margin_w method.gamma=1 +wadnb_tags=[grid_search_margin] method.margin_type=adaptive experiment=dev hydra=search &
  done
done
;;
logit_d)
