...
def log_sigmoid_meh(logits):
    ...
    return torch.clamp(torch.log(1.0 / (1.0 + torch.exp(-logits))), max=-1e-7,
                       min=-float('inf'))
...
def log_pr_exactly_k(logp, logq, k):
    ...
    state = torch.ones((batch_size, k + 2), device=logp.device) * -float('inf')
    state[:, 1] = 0
    ...
    for i in range(1, n + 1):
        state = torch.cat([
            torch.ones([batch_size, 1], device=logp.device) * -float('inf'),
            logaddexp(
                state[:, :-1] + logp[:, i - 1:i],
                state[:, 1:] + logq[:, i - 1:i]
            )
