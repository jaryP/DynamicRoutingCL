...
#  python main.py +scenario=cil_cifar10_5 model="$MODEL" +training=cifar10_5 +method=margin_cifar10 method.mem_size=200 head=margin_head method.gamma=1 +method.alpha=0 method.past_task_reg=0.05  device="$DEVICE" optimizer.momentum=0.9
  ...
  python main.py +scenario=cil_cifar10_5 model="$MODEL" +training=cifar10_5 +method=margin_cifar10 method.mem_size=1000 head=margin_head method.gamma=1 +method.alpha=0 method.past_task_reg=0.25 +method.rehearsal_metric='mse'  device="$DEVICE" +wadnb_tags=[final_results_margin]
