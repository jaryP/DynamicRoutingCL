...
future)
  ...
  do
    ...
    python main.py +scenario=cil_tyn_10 model=$MODEL +training=tinyimagenet +method=margin device=$DEVICE method.mem_size=2000 method.past_task_reg=0.25 method.gamma=1 hydra=search +wadnb_tags=[margin_future_ablation] head=margin_head +head.future_classes=$future_classes &
  done
;;
logit)
